-- =============================================================
-- 《飞鸽传书》微信小程序 V1.1（发送即起飞版）后端表结构
-- 模块：search-dao / search-service / search-smallapp
-- 约定：库表用蛇形命名；项目未开启 mapUnderscoreToCamelCase，
--       查询 SQL 必须用列别名显式映射驼峰属性（见 Entity/Mapper）。
-- 说明：若本地曾按 V1.0 建过表，请改用 feige_v1_migration.sql 做 ALTER。
-- =============================================================
...
CREATE TABLE IF NOT EXISTS `feige_letter` (
  `id`                     BIGINT       NOT NULL AUTO_INCREMENT COMMENT '主键',
  `letter_id`              VARCHAR(40)  NOT NULL                COMMENT '内部唯一ID(发件人/收件人确认后使用)',
  ...
  `sender_lat`             DECIMAL(14,8) DEFAULT NULL           COMMENT '发件纬度(仅用于距离,不对外)',
  ...
  `content`                TEXT                                 COMMENT '正文(≤500字,分享预览/未认领绝不返回)',
  `image_url`              VARCHAR(255) DEFAULT NULL            COMMENT '配图(分享预览/未认领绝不返回)',
  ...
  `claimed_at`             DATETIME     DEFAULT NULL            COMMENT '收件人认领成功时间',
  `recalled_at`            DATETIME     DEFAULT NULL            COMMENT '主动召回时间',
  `expired_at`             DATETIME     DEFAULT NULL            COMMENT '自动过期时间',
  `arrival_time`           DATETIME     DEFAULT NULL            COMMENT '预计到达=原departure+时长(认领后)',
  ...
  `settled`                TINYINT      NOT NULL DEFAULT 0      COMMENT '抵达成长是否已结算(0/1)',
...
-- 飞行日志（认领成功后按 letter_id/departure/arrival/distance 确定性生成,落库保证可回顾）
